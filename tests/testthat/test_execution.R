...
module4_1 <- CustomFunctionModule$new(name = "module4_1", fun = function(a,b,c){cat(a+b+c, file=file.path(outputDir, file = "workflow1_output.txt"))})
...
module1_2 <- CustomFunctionModule$new(name = "module1_2", fun = function(a,b){saveRDS(a+b, file=file.path(outputDir, file = "workflow2_output.rds"))})
...
test_that("Testing Execution Results Values", {
    capture.output(Execute(workflow2
                           ...
                           , mode = SERIAL_MODE))
    expect_equal({
       readRDS(file.path(outputDir, "workflow2_output.rds"))
    }, 15)
    expect_true(!any(grepl('_inputs.rds', list.files())))
    expect_true(!any(grepl('_inputs.rds', list.files(outputDir))))
    unlink(file.path(outputDir, file = "workflow2_output.rds"))

    capture.output(Execute(workflow2
                           , argsContainer = list(a = 4
                                                  ,b = -1)
                           , moduleInputsSaveDir = outputDir
                           , mode = PARALLEL_MODE))
    expect_equal({
        readRDS(file.path(outputDir, "workflow2_output.rds"))
    }, 3)
    inputsRDSFiles = list.files(outputDir)
    expect_true('module1_2_inputs.rds' %in% inputsRDSFiles)
    unserializedInputs <- readRDS(file.path(outputDir, file = "module1_2_inputs.rds"))
    ...
    unlink(file.path(outputDir, file = "workflow2_output.rds"))
    unlink(file.path(outputDir, file = "module1_2_inputs.rds"))

    capture.output(Execute(workflow2
                           , argsContainer = list(a = 4
                                                  ,b = -1)
                           , moduleInputsSaveDir = file.path(outputDir,'does_not_exist')
                           , mode = PARALLEL_MODE))
    inputsRDSFiles = list.files(file.path(outputDir, 'does_not_exist'))
    expect_true('module1_2_inputs.rds' %in% inputsRDSFiles)
    unlink(file.path(outputDir, file = "workflow2_output.rds"))
    unlink(file.path(outputDir, 'does_not_exist', file = "module1_2_inputs.rds"))
    unlink(file.path(outputDir, 'does_not_exist'))
})
